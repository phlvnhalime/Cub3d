...
/*   Updated: 2025/07/13 14:52:56 by julcalde         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	get_tex_color(mlx_texture_t *texture, int tex_x, int tex_y)
{
	...
	if (!texture || tex_x < 0 || tex_x >= (int)texture->width || \
		tex_y < 0 || tex_y >= (int)texture->height)
		return (0xFF00FFFF);
	pix = texture->pixels;
	r = pix[(tex_y * (int)texture->width + tex_x) \
		* texture->bytes_per_pixel];
	g = pix[(tex_y * (int)texture->width + tex_x) \
		* texture->bytes_per_pixel + 1];
	b = pix[(tex_y * (int)texture->width + tex_x) \
		* texture->bytes_per_pixel + 2];
	if (texture->bytes_per_pixel == 4)
		a = pix[(tex_y * (int)texture->width + tex_x) \
